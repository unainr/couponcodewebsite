...
export async function generateMetadata({
	...
	if (!matchedCategory) {
		return {
			title: "Category Not Found | RedeemlyNow",
			description:
				"The requested category could not be found. Browse our other categories for coupon codes and deals.",
		};
	}

	return {
		title: `${matchedCategory.name} Coupons & Promo Codes - Top ${matchedCategory.count} Stores | RedeemlyNow`,
		description: `Find the best ${matchedCategory.name} coupon codes, promo offers, and exclusive deals. Save money on ${matchedCategory.name} products and services with verified discounts from RedeemlyNow.`,
		keywords: `${matchedCategory.name} coupons, ${matchedCategory.name} promo codes, ${matchedCategory.name} deals, ${matchedCategory.name} discounts, ${matchedCategory.name} stores, RedeemlyNow`,
	};
}
...
async function getCategoryData(urlCategoryName: string) {
	...
	// Find the category whose URL-friendly name matches the parameter
	const matchedCategory = allCategories.find(
		(cat: any) => cat.slug === urlCategoryName
	);
