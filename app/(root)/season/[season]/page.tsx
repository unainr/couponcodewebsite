...
export async function generateMetadata({
	...
	if (!seasonData) {
		return {
			title: "Season Not Found | RedeemlyNow",
			description:
				"The requested seasonal deals could not be found. Browse our other seasonal offers for coupon codes and deals.",
		};
	}

	return {
		title: `${seasonData.name} Coupon Codes & Deals - Limited Time Offers | RedeemlyNow`,
		description: `Save big with exclusive ${seasonData.name} coupon codes, promo offers, and limited-time deals. Find the best ${seasonData.name} discounts from top brands at RedeemlyNow.`,
		keywords: `${seasonData.name} coupons, ${seasonData.name} deals, ${seasonData.name} offers, seasonal discounts, holiday deals, RedeemlyNow`,
	};
}
...
const SeasonPage = async ({
	...
	if (!seasonData) {
		return (
			<div className="container mx-auto px-4 py-8 flex flex-col min-h-screen items-center justify-center">
				Season not found
			</div>
		);
	}
